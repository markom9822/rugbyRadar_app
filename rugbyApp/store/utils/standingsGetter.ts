...
export const getAllStandingsData = (seasonStandings: any, playoffCutoffIndex: number | undefined) => {
...
export const getLeagueStandingsDataPlanetRugby = (seasonStandings: any, isPooled: boolean, isEndOfList: boolean, playoffCutoffIndex: number | undefined) => {
    ...
    for (let i = 0; i < standingsChildren.length; i++) {

        if (i === playoffCutoffIndex) {
            let headerRankingInfo = {
                isHeader: false,
                teamPool: '',
                teamName: 'Pool',
        ...
        const teamName = standingsChildren[i].team_name;

        const teamGP = standingsChildren[i].played;
        const teamWins = standingsChildren[i].won
        const teamDraws = standingsChildren[i].draw;
        const teamLosses = standingsChildren[i].lost;
        const teamPD = standingsChildren[i].points_diff;
        const teamPoints = standingsChildren[i].points;
        ...
        let newRankingInfo = {
            ...
            teamPoints: teamPoints,
            ranking: i,
            isLastItem: i === standingsChildren.length - 1,
            isEndOfList: isEndOfList && i === standingsChildren.length - 1,
            isPlayoffCutoff: false,
        };
